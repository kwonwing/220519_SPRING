...
//        member 가 null이면 회원정보가 없는것
//        패스워드를 변경할수 없으므로 에러를 발생시키고 메소드를 빠져나옴
        if(member == null) {
            throw new MemberNotFoundException();
        }

//        에러가 없으면(member가 null이 아니면) 회원정보가 있으므로
//        패스워드 변경 서비스 실행( 구패스워드 -> 신패스워드 변경 )
